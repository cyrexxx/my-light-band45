...
{   
   ble_gatts_evt_write_t * p_evt_write = &p_ble_evt->evt.gatts_evt.params.write;
	...
	if ((p_evt_write->handle == p_time->time_high_char_handles.value_handle ) &&
      (p_evt_write->len == 1) && 
      (p_time->time_high_write_handler != NULL))
					{
						p_time->time_high_write_handler(p_time,p_evt_write->data[0]);	
						
					}
  else if ((p_evt_write->handle == p_time->time_low_char_handles.value_handle ) &&
      (p_evt_write->len == 1) && 
      (p_time->time_low_write_handler != NULL))
					{
						p_time->time_low_write_handler(p_time,p_evt_write->data[0]);	
					}				
 ...
 */
static uint32_t current_time_high_char_add(ble_time_t * p_time, const ble_time_init_t * p_time_init)
{
    ...
    ble_uuid.uuid = TIME_UUID_CURRENT_TIME_HIGH_CHAR;
 ...
 * @return      NRF_SUCCESS on success, otherwise an error code.
 */
static uint32_t current_time_low_char_add(ble_time_t * p_time, const ble_time_init_t * p_time_init)
{
    ble_gatts_char_md_t char_md;
    ble_gatts_attr_t    attr_char_value;
    ble_uuid_t          ble_uuid;
    ...
    ble_uuid.type = p_time->uuid_type;
    ble_uuid.uuid = TIME_UUID_CURRENT_TIME_LOW_CHAR;
...
uint32_t ble_time_init(ble_time_t * p_time, const ble_time_init_t * p_time_init)
{
    ...
    p_time->evt_handler               = p_time_init->evt_handler;
	  p_time->time_high_write_handler   = p_time_init->time_high_write_handler;
	  p_time->time_low_write_handler    = p_time_init->time_low_write_handler;
    p_time->conn_handle               = BLE_CONN_HANDLE_INVALID;
    ...
    // Add current time characteristic
	  err_code= current_time_high_char_add(p_time, p_time_init);
		if (err_code != NRF_SUCCESS)
    {
        return err_code;
    }
		 err_code= current_time_low_char_add(p_time, p_time_init);
		if (err_code != NRF_SUCCESS)
    {
        return err_code;
    }
...
uint32_t ble_time_time_high_update(ble_time_t * p_time, uint8_t time)
{
    ...
    uint16_t len = sizeof(uint8_t);
        ...
        // Update database
  return sd_ble_gatts_value_set(p_time->time_high_char_handles.value_handle,
                                          0,
                                          &len,
                                          &time);
}
...
uint32_t ble_time_time_low_update(ble_time_t * p_time, uint8_t time)
{
    ...
    uint16_t len = sizeof(uint8_t);
        ...
        // Update database
   return sd_ble_gatts_value_set(p_time->time_low_char_handles.value_handle,
                                          0,
                                          &len,
                                          &time);
}
